...
$r2 = $this->curlx->Get('https://sso.teachable.com/secure/1200231/checkout/3781536/nutrition-certification-complete-certification-course-2022', null, $cookie, $server['proxy']);
...
$checkout = trim(strip_tags($this->getstr($r2->body, 'data-checkout="https://sso.teachable.com/secure/1200231/checkout/', '/')));
...
$headers = [
	...
	'referer: https://sso.teachable.com/secure/1200231/checkout/'.$checkout.'/nutrition-certification-complete-certification-course-2022',
...
$r3 = $this->curlx->Custom('https://sso.teachable.com/secure/1200231/student_checkout/'.$order.'/accounts/verify_email.json', 'PATCH', $data, $headers, $cookie, $server['proxy']);
...
$r4 = $this->curlx->Post('https://sso.teachable.com/secure/1200231/student_checkout/'.$order.'/confirm/credit_card.json', $data, $headers, $cookie, $server['proxy']);
...
if (isset($json_r4->redirect_to)) {
	do {
		...
		$r5 = $this->curlx->Get('https://sso.teachable.com/secure/1200231/student_checkout/'.$order.'/check.json', $headers, $cookie, $server['proxy']);
